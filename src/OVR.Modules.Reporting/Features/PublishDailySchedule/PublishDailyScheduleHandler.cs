...
public sealed class PublishDailyScheduleHandler(
    IReportGenerator generator,
    IReportRecordRepository recordRepository,
    IS3Storage s3Storage)
    : IRequestHandler<PublishDailyScheduleCommand, ErrorOr<PublishReportResponse>>
{
    public async Task<ErrorOr<PublishReportResponse>> Handle(
        PublishDailyScheduleCommand request, CancellationToken cancellationToken)
    {
